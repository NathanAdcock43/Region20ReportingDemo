-- 1) Insert or update a score, then return updated course percentage (capped at 100%)
...
-- Aggregate per assessment, cap each ratio at 1.0, then weight and scale to %
RETURN QUERY
    WITH per_assessment AS (
    SELECT a.assessment_id,
           LEAST( COALESCE(SUM(s.points_earned) / NULLIF(SUM(s.points_possible),0), 0), 1.0 ) AS ratio,
           a.weight_pct
...
-- 2) Per-course progress for a student and term (also caps at 100%)
CREATE OR REPLACE FUNCTION app.usp_get_student_progress(p_student_id INT, p_term TEXT)
RETURNS TABLE (course_name TEXT, grade_pct NUMERIC)
LANGUAGE sql
AS $$
WITH enroll AS (
  SELECT e.course_id
...
scores_agg AS (
  -- collapse to one ratio per assessment, cap each at 1.0
